...
State exactly what Warden does and does not protect — over- and under-claiming are both failures.
...
- **Mitigation (optional):** trigger-gated identity (a "Layer 0′" pattern, as in Maktub's Veil) forces such an attack to be a **loud, detectable blanket release** rather than a quiet targeted one.
...
Opening an envelope POSTs the **condition** (which, for a Veil-style consumer, embeds `beatId` + contract address + chainId) to every node's `/partial`. So each node operator — and any on-path observer of that request — learns **requester-IP ↔ condition interest** and the *timing* of the open attempt. The condition (and any `beatId`) is already public on-chain; what's new is binding it to a requesting IP at open time. This is consistent with "not metadata-private" below, not a content leak (a tampered condition simply never releases — fail-closed). Mitigations for the independent-operator federation: client egress over a privacy-preserving transport (e.g. Tor/oblivious relay), and TLS pinning / a known-key channel to deny passive collectors the linkage. **Preview note:** with the all-ours testnet federation this is moot, but it MUST be tracked before independent operators. (Consumer-side concern — e.g. Maktub's `_fetchPartial` in its Veil client.)
...
- **Not metadata-private.** Existence, timing, condition, and (for a Veil-style consumer) `beatId` are public. Warden hides *content until the trigger*, never *that something exists*.
