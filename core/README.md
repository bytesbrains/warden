...
Core cryptography for **Warden** — the event-gated threshold conditional-decryption network. Any app needing "decrypt only when this on-chain condition holds" can build on it (Maktub's Veil delivery layer is one consumer). Phase 0 PoC.
...
| `envelope` | The **`warden-v1` double-wrap** (`seal`/`open`): AEAD content + ECIES recipient gate + threshold-IBE condition gate. Plus the **`warden-gate-v1` condition-gate-only** primitive (`seal_gated`/`open_gated`) that wraps an *already-encrypted* blob (e.g. an app's existing hybrid envelope, as Maktub's Veil does) — Warden contributes timing, the host app keeps recipient confidentiality. JSON wire form. |
...
- The node daemon + Base condition-watcher and client CLI — both consume `fed` files; WASM/Dart-FFI client targets later.
- Reconcile `ecies` byte format with a consuming app's existing recipient-registry ECIES before integration (e.g. Maktub's `RecipientRegistry`).
